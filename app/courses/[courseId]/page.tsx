...
import { doc, getDoc, addDoc, collection, query, where, getDocs, Timestamp, serverTimestamp } from 'firebase/firestore';
import { db } from '@/lib/firebase/config';
import { Course, COLLECTIONS, CourseLesson, Enrollment, EnrollmentStatus, CourseDifficulty } from '@/types';
...
export default function CourseDetailPage() {
    ...
    const handleEnrollClick = async (e: React.MouseEvent) => {
        ...
        // Handle Free Course
        if (course?.pricing.threeMonths === 0) {
            e.preventDefault();

            if (confirm('ยืนยันการลงทะเบียนคอร์สฟรี? คุณจะสามารถเข้าเรียนได้ทันที')) {
                try {
                    const startDate = new Date();
                    const expiresAt = new Date(startDate);
                    expiresAt.setMonth(expiresAt.getMonth() + 12); // Give 12 months (or default max) for free courses
                    ...
                    const enrollmentData = {
                        ...
                        accessGranted: true, // Immediate access
                        paymentSlipId: 'free_course',
                        pricePaid: 0,
                        progress: [],
                        overallProgress: 0,
                    ...
                    toast({
                        ...
                        description: 'คุณสามารถเข้าเรียนคอร์สนี้ได้แล้ว',
                    });

                    // Refresh enrollment state or redirect
                    router.push(`/learn/course/${course.id}`);
                } catch (error) {
                    console.error('Error auto-enrolling in free course:', error);
                    toast({
                        title: 'เกิดข้อผิดพลาด',
                        description: 'ไม่สามารถลงทะเบียนคอร์สเรียนได้ในขณะนี้',
                        variant: 'destructive',
                    });
                }
            }
    ...
    const difficultyConfig: Record<string, { bg: string; text: string; label: string }> = {
        [CourseDifficulty.BEGINNER]: { bg: 'bg-green-500', text: 'text-white', label: 'ปรับพื้นฐาน' },
        [CourseDifficulty.INTERMEDIATE]: { bg: 'bg-golden', text: 'text-ink-black', label: 'ติวสอบเข้ามหาลัยฯ' },
        [CourseDifficulty.ADVANCED]: { bg: 'bg-fighter-red', text: 'text-white', label: 'ติวเข้มข้น/ข้อสอบปราบเซียน' },
    };
    const difficulty = difficultyConfig[course.difficulty || CourseDifficulty.BEGINNER] || difficultyConfig[CourseDifficulty.BEGINNER];
                            ...
                            <div className="bg-white border-4 border-black p-6 shadow-[12px_12px_0_rgba(0,0,0,1)] relative">
                                <div className="text-center mb-6">
                                    ...
                                    <div className="text-4xl font-heading text-fighter-red">
                                        {course.pricing.threeMonths === 0 ? 'ฟรี (0 บาท)' : formatCurrency(course.pricing.threeMonths)}
                                    </div>
                                    <p className="text-xs font-bold uppercase text-gray-400">/ {course.pricing.threeMonths === 0 ? '12 เดือน' : '3 เดือน'}</p>
                                </div>
