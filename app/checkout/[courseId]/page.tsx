...
export default function CheckoutPage() {
    ...
    const handleSubmit = async () => {
        ...
        if (selectedPrice > 0 && (!paymentData.amount || !paymentData.paymentDate || !paymentData.paymentTime)) {
            toast({
                title: 'Error',
        ...
        try {
            ...
            if (selectedPrice > 0) {
                ...
                slipImageUrl = await getDownloadURL(storageRef);
    ...
    return (
        <div className="min-h-screen py-12 bg-ink-black text-white">
            <div className="container mx-auto px-4 max-w-4xl">
                ...
                {currentStep === 'payment' && selectedPrice > 0 && (
                    <PaymentStep
    ...
    return (
        <Card className="glass-card p-8">
            <h2 className="text-2xl font-bold text-neon-purple mb-6">
                {selectedPrice === 0 ? 'Confirm Free Enrollment' : 'Upload Payment Slip'}
            </h2>
            ...
            <div className="space-y-6">
                {selectedPrice > 0 ? (
                    ...
                    <div className="text-center py-12 border-2 border-dashed border-white/10 rounded-lg bg-white/5">
                        <Zap className="w-16 h-16 text-fighter-red mx-auto mb-4 drop-shadow-[0_0_15px_rgba(255,0,0,0.5)]" />
                        <h3 className="text-2xl font-bold mb-2">Free Enrollment</h3>
                        <p className="text-dark-text-secondary text-sm">Click the button below to confirm your free enrollment. You will gain immediate access to the course content.</p>
                    </div>
                )}
            </div>
                ...
                >
                    {submitting ? (
                        <>
                            ...
                            {selectedPrice === 0 ? 'Enrolling...' : 'Submitting...'}
                        </>
                    ) : (
                        <span className="flex items-center gap-2">
                            {selectedPrice === 0 ? <Zap className="w-4 h-4" /> : <CreditCard className="w-4 h-4" />}
                            {selectedPrice === 0 ? 'Confirm Free Enrollment' : 'Submit Payment'}
                        </span>
                    )}
                </Button>
            </div>
